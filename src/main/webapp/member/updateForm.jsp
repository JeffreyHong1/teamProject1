...
<head>
	...
	<form action="updatePro.jsp" method="post" name="update" onsubmit="return checkValue();">
	...
	<script>
		function checkValue(){
				...
				if(!pwptn.test(document.update.pw.value)){
					alert("비밀번호는 5~10자리,숫자,영문자가 포함되어야합니다.");
					document.update.pw.value="";
					document.update.pw.focus();
					return false;
				}
				p1 = document.update.pw.value;
			    p2 = document.update.pw2.value;
			    if( p1 != p2  ){
					alert("pw가 일치하지 않습니다.");
					document.update.pw.focus();
					return false;
				...
				ph = document.update.phone2.value.length;
				if(ph != 11){
					alert("연락처를 입력해주세요.");
					document.update.phone2.focus();
					return false;
				}
				e = document.update.email1.value;
				if(e == ""){
					alert("이메일을 입력해주세요.");
					document.update.email1.focus();
					return false;
				}
				a1 = document.update.addr1.value;
				if(a1 == ""){
					alert("주소를 입력해주세요.");
					document.update.addr1.focus();
					return false;
				}
				a3 = document.update.addr3.value;
				if(a3 == ""){
					alert("상세주소를 입력해주세요.");
					document.update.addr3.focus();
					return false;
				}
			}
		</script>
