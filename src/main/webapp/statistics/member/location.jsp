...
<body>
	...
	<%
		String location = request.getParameter("member2");		
		...
		String locCount = dao.locCount(location);
		int count = Integer.parseInt(locCount);
		
		
		if(select.equals("2")){			
			ArrayList<adminMemDTO> list = dao.locSearch(location,start,end);			
				if(list.size() > 0){
	...
	<div style="width: 100%; margin: auto; text-align: center; display: inline-block; padding-top: 20px;">
		...
		%>
					<br/>
					<a href="location.jsp?pageNum=<%=startPage - 10 %>&select=<%=select%>&member2=<%=location%>">[이전]</a>		
		<%		}
				for (int i = startPage; i <= endPage; i++){
		%>			
					<a href="location.jsp?pageNum=<%=i %>&select=<%=select%>&member2=<%=location%>">[<%=i %>]</a>	
		<%		}
				if(endPage < pageCount){
		%>	
					<a href="location.jsp?pageNum=<%=startPage + 10%>&select=<%=select%>&member2=<%=location%>">[다음]</a>
		<%		}	
			}
		%>
	</div>
