...
<body>
	<jsp:include page="/statistics/header.jsp"></jsp:include>
		...
		<%
			...
			String num1 = request.getParameter("date1");
			String num2 = request.getParameter("date1_1");
			...
			String ageCount = dao.orderDateCount(num1, num2);			
		...
		<table border="1" class="type09">
			...
			<%				
				if(select.equals("2")){					
					ArrayList<orderStatDTO> list = dao.dateSearch(num1,num2,start,end);
					if(list.size() > 0){
		...
		<div style="width: 100%; margin: auto; text-align: center; display: inline-block; padding-top: 20px;">
			...
			%>
						<br/>
						<a href="orderDate.jsp?pageNum=<%=startPage - 10 %>&select=<%=select%>&date1=<%=num1%>&date1_1=<%=num2%>">[이전]</a>		
			<%		
					}
					for (int i = startPage; i <= endPage; i++){
			%>			
						<a href="orderDate.jsp?pageNum=<%=i %>&select=<%=select%>&date1=<%=num1%>&date1_1=<%=num2%>">[<%=i %>]</a>	
			<%		
					}
					if(endPage < pageCount){
			%>	
						<a href="orderDate.jsp?pageNum=<%=startPage + 10%>&select=<%=select%>&date1=<%=num1%>&date1_1=<%=num2%>">[다음]</a>
			<%		
					}	
				}
			%>
